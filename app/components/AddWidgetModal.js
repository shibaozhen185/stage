...
export default class AddWidgetModal extends Component {
    ...
    render() {
        const addWidgetBtn = <Button icon="bar chart" labelPosition='left' basic content='添加组件' className="addWidgetBtn"/>;

        const installWidgetBtn = <Button animated="vertical" id="installWidgetBtn" onClick={()=> {}}>
                                    <Button.Content visible>安装新组件</Button.Content>
                                    <Button.Content hidden>
                                        <Icon name="folder open" />
                                    </Button.Content>
                                </Button>;

        const updateWidgetBtn = <Button floated='left' size="small" compact basic content="更新"
        ...
        const confirmContent = !_.isEmpty(this.state.usedByList) ?
            (<Segment basic>
                ...
                <DataTable>
                <DataTable.Column label="用户名"/>
                <DataTable.Column label="管理员 IP"/>
            ...
            <div className={this.props.className}>
                <Modal trigger={addWidgetBtn} className="addWidgetModal" open={this.state.open} closeIcon
                    ...
                    <Segment basic size="large">
                        ...
                        <Input icon='search' fluid size="mini" placeholder='搜索组件...'
                ...
                <Grid columns={2}>
                    <Grid.Row>
                        ...
                        <Item.Group divided className="widgetsList">
                                                ...
                                                <Item.Extra>
                                                    {widget.isCustom &&
                                                        <div>
                                                            ...
                                                            <Button floated='left' size="small" compact basic content="移除"
                            ...
                            {_.isEmpty(this.state.filteredWidgetDefinitions) && <Item className="alignCenter" content="无查询结果"/>}
                        </Item.Group>
                        ...
                        <Button.Group widths='2'>
                            <Button animated="vertical" id="addWidgetsBtn" onClick={() => {this._addWidgets()}} color="green"
                                ...
                                <Button.Content visible>添加所选组件 ({this.state.widgetsToAdd.length})</Button.Content>
                                <Button.Content hidden>
                                    <Icon name="check" />
                                </Button.Content>
                            </Button>
                ...
                <Confirm open={this.state.showConfirm}
                         ...
                         header={`确定要移除业务 ${this.state.widget.name}?`}
