...
export default class CreatePageModal extends React.Component {
    ...
    render() {
        ...
        const trigger = <Button content='创建页面' icon='block layout' labelPosition='left' className='createPageButton'/>;
        ...
        return (
            <Modal trigger={trigger} open={this.state.open} onOpen={this._openModal.bind(this)}
                ...
                <Modal.Content>
                    <Form loading={this.state.loading} errors={this.state.errors} onErrorsDismiss={() => this.setState({errors: {}})}>
                        <Form.Field error={this.state.errors.pageName}>
                            <Form.Input name='pageName' placeholder="页面名"
                ...
                <Modal.Actions>
                    ...
                    <ApproveButton onClick={this._submitCreate.bind(this)} disabled={this.state.loading} content='创建' icon="checkmark" color="green"/>
                </Modal.Actions>
            </Modal>
        );
    }
