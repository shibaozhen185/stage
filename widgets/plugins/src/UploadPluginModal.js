...
export default class UploadModal extends React.Component {
    ...
    _submitUpload() {
        ...
        if (_.isEmpty(this.state.wagonUrl) && !wagonFile) {
            errors['wagon']='请选择wagon文件或url';
        }

        if (!_.isEmpty(this.state.wagonUrl) && wagonFile) {
            errors['wagon']='必须选择wagon车文件或url，而不是两者都选';
        }

        if (_.isEmpty(this.state.yamlUrl) && !yamlFile) {
            errors['yaml']='请选择配置文件或url';
        }

        if (!_.isEmpty(this.state.yamlUrl) && yamlFile) {
            errors['yaml']='必须选择配置文件或url，而不是两者都选';
        }
        ...
        return (
            <Modal trigger={uploadButton} open={this.state.open} onOpen={()=>this.setState({open:true})} onClose={()=>this.setState({open:false})}>
                    ...
                    <Form loading={this.state.loading} errors={this.state.errors}
                        ...
                        <Form.Group>
                            <Form.Field width="9" error={this.state.errors.wagon}>
                                <Form.Input label="URL" placeholder="键入连接" name="wagonUrl"
                            ...
                            <Form.Field width="8" error={this.state.errors.wagon}>
                                <Form.File placeholder="选择wagon文件" name="wagonFile" ref="wagonFile"
                        ...
                        <Form.Group>
                            <Form.Field width="9" error={this.state.errors.yaml}>
                                <Form.Input label="URL" placeholder="键入连接" name="yamlUrl"
                            ...
                            <Form.Field width="8" error={this.state.errors.yaml}>
                                <Form.File placeholder="选择配置文件" name="yamlFile" ref="yamlFile"
                ...
                <Modal.Actions>
					...
					disabled={this.state.loading} content="上传" icon="upload" color="green"/>
                </Modal.Actions>
            </Modal>
        );
    }
