        ...
        return (
            <div>
                ...
                <DataTable fetchData={this.fetchGridData.bind(this)}
                    ...
                    <DataTable.Column label="模版" name="blueprint_id" width="20%"
                                 show={fieldsToShow.indexOf('Blueprint') >= 0 && !this.props.data.blueprintId}/>
                    <DataTable.Column label="部署" name="deployment_id" width="20%"
                                 show={fieldsToShow.indexOf('Deployment') >= 0 && !this.props.data.deploymentId}/>
                    <DataTable.Column label="工作流" name="workflow_id" width="15%"
                                 show={fieldsToShow.indexOf('Workflow') >= 0}/>
                    <DataTable.Column label="编号" name="id" width="15%"
                                 show={fieldsToShow.indexOf('Id') >= 0}/>
                    <DataTable.Column label="创建时间" name="created_at" width="10%"
                                 show={fieldsToShow.indexOf('Created') >= 0}/>
                    <DataTable.Column label="创建者" name='created_by' width="5%"
                                      show={fieldsToShow.indexOf('Creator') >= 0}/>
                    <DataTable.Column label="系统运行" name="is_system_workflow" width="5%"
                                 show={fieldsToShow.indexOf('IsSystem') >= 0}/>
                    <DataTable.Column label="参数" name="parameters" width="5%"
                                 show={fieldsToShow.indexOf('Params') >= 0}/>
                    <DataTable.Column label="状态" width="5%" name="status"
