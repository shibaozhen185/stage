                        ...
                        return (
                            <DataTable.Row key={item.id} selected={item.isSelected} onClick={()=>this.props.onSelect(item)}>
                                ...
                                <DataTable.Data className="center aligned rowActions">
                                    <Icon name="info" link title="模板描述" loading={this.props.readmeLoading === item.name}
                                          onClick={(event)=>{event.stopPropagation();this.props.onReadme(item.name)}} bordered/>
                                    <Icon name="upload" link title="上传模板" onClick={(event)=>{event.stopPropagation();this.props.onUpload(item.name)}} bordered/>
                                </DataTable.Data>
                            </DataTable.Row>
                        );
                    })
