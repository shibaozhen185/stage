    ...
    render(){
        ...
        return (
            <DataTable fetchData={this.props.fetchGridData}
                ...
                <DataTable.Column label="模版名称" name="id" width="20%"/>
                <DataTable.Column label="创建时间" name="created_at" width="15%"/>
                <DataTable.Column label="修改时间" name="updated_at" width="15%"/>
                <DataTable.Column label="创建者" name='created_by' width="15%"/>
                <DataTable.Column label="主文件名" name='main_file_name' width="15%"/>
                <DataTable.Column label="部署数量" width="10%"/>
                        ...
                        return (
                            <DataTable.Row id={`${tableName}_${item.id}`} key={item.id} selected={item.isSelected} onClick={()=>this.props.onSelectBlueprint(item)}>
                                ...
                                <DataTable.Data className="center aligned rowActions">
                                    <i className="rocket icon link bordered" title="创建部署" onClick={(event)=>{event.stopPropagation();this.props.onCreateDeployment(item)}}></i>
                                    <i className="trash icon link bordered" title="删除模板" onClick={(event)=>{event.stopPropagation();this.props.onDeleteBlueprint(item)}}></i>
                                </DataTable.Data>
                            </DataTable.Row>
                        );
                    })
