...
export default class TenantsTable extends React.Component {
    ...
    _selectAction(value, tenant) {
        if (value === MenuAction.EDIT_USERS_ACTION) {
            ...
            this.setState({error: `内部错误：未知操作 ('${value}') 无法处理.`});
        }
    }
        ...
        return (
            <div>
                ...
                <DataTable fetchData={this.fetchGridData.bind(this)}
                    ...
                    <DataTable.Column label="租户名称" name="name" width="30%" />
                    <DataTable.Column label="用户组" width="30%" />
                    <DataTable.Column label="用户" width="30%" />
                ...
                <DeleteModal content={`你确定你想要删除租户 '${this.state.tenant.name}'?`}
